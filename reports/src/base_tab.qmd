...
basetabfunc <- function(data, colvar, novars = NULL) {
  tab <- data %>%
    count(!!sym(colvar), .drop = F) %>%
    mutate(n = paste0(n, " (", fn(n / sum(n) * 100, catdig), ")")) %>%
    pivot_wider(names_from = !!sym(colvar), values_from = n) %>%
    mutate(
      var = "N (%)",
      level = NA,
      p = NA,
    ...
    select(var, level, pmis, levels(data %>% pull(!!sym(colvar))), p)

  tabvarstmp <- setdiff(tabvars, novars)
  tabtmp <- sapply(tabvarstmp,
    FUN = basetabfunc_inner, data = data,
    stratavar = colvar, catdig = catdig, contdig = contdig
  )
  ...
  tab <- left_join(tab,
    ...
    mutate(
      ...
      No = if_else(Variable == "Palliative care given", "-", No)
    )
  ...
  cn <- c("Variable", "Missing (%)", levels(data %>% pull(!!sym(colvar))), "p-value")
  ...
  tab_xls <- tab %>%
    ...
    add_row(Variable = paste0("c ", footnotetext[2]))
  ...
  default_kable(tab,
    ...
    add_header_above(c(" " = 1, " " = 1, "Palliative care" = 2, " " = 1)) %>%
    footnote(
...
```{r}
#| label: tbl-base-indicated
#| cache: true
#| cache.comments: false
#| dependson: basetabfunc
#| tbl-cap: "Baseline characteristics by palliatve care indicated"
#| tbl-pos: "H"

basetabfunc(data = rsdata, colvar = "shf_palliative_indicated")
```

```{r}
#| label: tbl-base-given
#| cache: true
#| cache.comments: false
#| dependson: basetabfunc
#| tbl-cap: "Baseline characteristics by palliatve care given"
#| tbl-pos: "H"

basetabfunc(data = rsdata %>% filter(!is.na(shf_palliative_given)), colvar = "shf_palliative_given", novars = "shf_palliative_given")
```

```{r}
#| label: tbl-base-indicatedgiven
#| cache: true
#| cache.comments: false
#| dependson: basetabfunc
#| tbl-cap: "Baseline characteristics for patients with palliative care indicated by palliatve care given"
#| tbl-pos: "H"

basetabfunc(
  data = rsdata %>% filter(!is.na(shf_palliative_given) & shf_palliative_indicated == "Yes"),
  colvar = "shf_palliative_given", novars = "shf_palliative_given"
)
```
