...
vars <- bind_rows(
  tibble(var = c("shf_palliative_indicated", tabvars)),
  outvars %>%
...
vars <- vars %>%
  mutate(
    ...
    Code = if_else(Variable == "Hospitalization", NA_character_, Code),
  )
...
vars <- vars %>%
  mutate(
    ...
    footnote3 = if_else(str_detect(var, "hosp") & str_detect(Variable, "Hosp", negate = T), footnote_marker_alphabet(3, format = "latex"), ""),
...
vars_xls <- vars %>%
  ...
  add_row(Variable = paste0("b ", footnotetext[4])) %>%
  add_row(Variable = paste0("b ", footnotetext[5]))
